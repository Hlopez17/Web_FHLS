  ...
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-background rounded-lg border border-border shadow-lg w-full max-w-md">
      <div class="p-6">
        ...
        <!-- Validación de inventarios asociados -->
        <div v-if="bodega.inventarios && bodega.inventarios.length > 0" class="bg-destructive/10 border border-destructive/20 rounded-md p-3 mb-4">
          ...
          <p class="text-destructive text-xs mt-1">
            ...
            Debe eliminar primero los inventarios antes de poder eliminar esta bodega.
          </p>
        </div>

        <p v-else class="text-muted-foreground text-center mb-6">
          ¿Estás seguro de que deseas eliminar la bodega 
          <span class="font-medium text-foreground">"{{ bodega.Nombre_bodega || 'Sin nombre' }}"</span>? 
        ...
        <div class="flex justify-center space-x-3">
          ...
          <Button
            ...
            @click="confirmDelete"
            class="px-6"
            :disabled="processing || (bodega.inventarios && bodega.inventarios.length > 0)"
          >
...
const confirmDelete = async () => {
  if (props.bodega.inventarios && props.bodega.inventarios.length > 0) {
    return;
  }
