    ...
    <div class="bg-background rounded-lg border border-border shadow-lg w-full max-w-md">
      <div class="p-6">
        ...
        <p class="text-muted-foreground text-center mb-6">
          ...
          <span class="font-medium text-foreground">"{{ user.name }} {{ user.Apellido || '' }}"</span>? 
...
const confirmDelete = async () => {
  ...
  try {
    await router.delete(`/usuarios/${props.user.Idusuario}`, {
      preserveScroll: true,
      onSuccess: () => {
        emit('deleted',`Usuario eliminado correctamente`,'success');
        emit('close');
      ...
      onError: (errors) => {
        emit('deleted', 'Ocurrió un error al eliminar al usuario', 'error')
